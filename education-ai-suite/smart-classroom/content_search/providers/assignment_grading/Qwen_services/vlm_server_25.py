"""VLM API Server using Qwen2.5-VL-3B-Instruct with OpenVINO"""
...
# Qwen2.5-VL-3B 模型路径 (int8 量化版本)
MODEL_PATH = os.path.join(SCRIPT_DIR, "..", "..", "..", "..", "models", "openvino", "Qwen2.5-VL-3B-Instruct", "int8")
MODEL_PATH = os.path.normpath(MODEL_PATH)

DEVICE = "GPU.1"
PORT = 9900  # 与 vlm_server.py 相同端口（同时只能运行一个）
...
# FastAPI应用
app = FastAPI(
    title="VLM API Server (Qwen2.5-VL-3B)",
    description="OpenAI-compatible API for Qwen2.5-VL-3B-Instruct",
    version="1.0.0",
    lifespan=lifespan
)
...
def load_model():
    ...
    logger.info("=" * 60)
    logger.info("Model Loading - Qwen2.5-VL-3B-Instruct")
    logger.info("=" * 60)
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        logger.error("Please ensure Qwen2.5-VL-3B-Instruct model is available")
        sys.exit(1)
    ...
    for file in critical_files:
        ...
        if os.path.exists(file_path):
            ...
            logger.info(f"Found: {file} ({file_size_gb:.2f} GB)")
        else:
            logger.warning(f"File not found: {file}")

    ov_cache_dir = os.path.join(SCRIPT_DIR, "ov_cache_25")
...
if __name__ == '__main__':
    print("=" * 60)
    print("VLM API Server - Qwen2.5-VL-3B-Instruct")
    print("=" * 60)
    print(f"Starting server on http://127.0.0.1:{PORT}")
    print(f"")
    print(f"Model:  Qwen2.5-VL-3B-Instruct (3B parameters)")
