...
class PaddleOCRVLService:
    def __init__(self, model_path, device="CPU", llm_int4_compress=False, llm_int8_compress=True,
                 vision_int8_quant=False, llm_int8_quant=True, repetition_penalty=1.3):
        self.model_path = Path(model_path)
        if not self.model_path.exists():
            raise FileNotFoundError(f"Model not found: {model_path}")

        self.device = device
        # Penalize repeated tokens to break greedy-decoding repetition loops
        # (e.g. "购购购...") that occur on low-quality / uncertain regions and
        # otherwise run until max_new_tokens. 1.0 = off; ~1.2-1.3 = mild.
