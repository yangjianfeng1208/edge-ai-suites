...
#
"""GPU device configuration tests for the Multimodal Weld Defect Detection sample app (Docker).

Sequence (chronological):
    1. Deploy multimodal stack (`make up`).
    2. POST GPU UDF config to Time Series Analytics microservice
       (`docker_utils.execute_multimodal_gpu_config_curl`).
    3. POST GPU model device to DL Streamer Pipeline Server pipeline
...
def test_gpu_multimodal(setup_multimodal_environment):
    """TC_GPU_MM_01: Multimodal GPU inference flow (TSA UDF + DLSPS pipeline) end-to-end."""
    logger.info("TC_GPU_MM_01: Multimodal GPU inference flow (Docker)")
    _run_multimodal_gpu_flow(setup_multimodal_environment, device="GPU")
