...
"""Tests for the rvc launch file structure."""
...
def test_launch_file_declares_use_sim_time():
    ...
    with open(LAUNCH_FILE, "r", encoding="utf-8") as fh:
        source = fh.read()
    assert "use_sim_time" in source, "Expected 'use_sim_time' argument in launch file"
...
def test_launch_file_declares_launch_stack():
    ...
    with open(LAUNCH_FILE, "r", encoding="utf-8") as fh:
        source = fh.read()
    assert "launch_stack" in source, "Expected 'launch_stack' argument in launch file"
