...
"""Performance test for RT-DETR Object Detection on Intel via OpenVINO."""
import os
import time
...
NETWORK_RESOLUTION = Resolution(640, 640)
...
def launch_setup(container_prefix, container_sigterm_timeout):
    ...
    model_path = os.path.join(assets_root, 'models', 'sdetr', 'sdetr_grasp.xml')
    ns = TestRtDetrIntel.generate_namespace()
    nodes = [
        ...
        ComposableNode(name='RtDetrOpenVINONode', namespace=ns, package='isaac_ros_benchmark',
            plugin='isaac_ros_benchmark::RtDetrOpenVINONode',
            parameters=[{'model_path': model_path, 'score_threshold': 0.5,
                         'network_size': 640, 'openvino_device': OV_DEVICE,
...
def generate_test_description():
    return TestRtDetrIntel.generate_test_description_with_nsys(launch_setup)

class TestRtDetrIntel(ROS2BenchmarkTest):
    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS® RT-DETR Benchmark (Intel OpenVINO)',
